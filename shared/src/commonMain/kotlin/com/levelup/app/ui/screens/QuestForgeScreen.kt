...
@Composable
fun QuestForgeScreen(state: LevelUpAppState) {
    val sState by state.suggestionState.collectAsState()
    val provider by state.aiProvider.collectAsState(initial = "offline")
    val hasKey by state.aiApiKey.collectAsState(initial = "")
            ...
            Surface(shape = RoundedCornerShape(20.dp), color = MaterialTheme.colorScheme.surface, modifier = Modifier.fillMaxWidth()) {
                Column(Modifier.padding(18.dp)) {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Box(
                            modifier = Modifier.size(36.dp).background(MaterialTheme.colorScheme.primary.copy(alpha = 0.15f), shape = RoundedCornerShape(50)),
                            contentAlignment = Alignment.Center
                        ) {
                            Icon(Icons.Filled.AutoAwesome, contentDescription = null, tint = MaterialTheme.colorScheme.primary)
                        }
                        Spacer(Modifier.width(10.dp))
                        Text("QUEST FORGE", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Black, color = MaterialTheme.colorScheme.onSurface)
                    }
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Button(
                            onClick = { state.generateQuests(goal) },
                            enabled = goal.isNotBlank() && sState !is SuggestionUiState.Loading,
                            colors = ButtonDefaults.buttonColors(
                                containerColor = MaterialTheme.colorScheme.primary,
                                contentColor = MaterialTheme.colorScheme.onPrimary
                            )
                        ) {
                            if (sState is SuggestionUiState.Loading) {
            ...
            is SuggestionUiState.Ready -> {
                item {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        IconButton(onClick = { state.generateQuests(s.goal) }) {
                            Icon(Icons.Filled.Refresh, contentDescription = "Refresh")
                        }
                        TextButton(onClick = { state.clearSuggestions() }) {
                            Text("CLEAR")
                        }
                    }
                }
                items(s.result.suggestions) { suggestion ->
                    SuggestionRow(suggestion = suggestion, onAccept = { state.saveSuggestion(it) })
                }
            }
        }
    }
    ...
    Surface(shape = RoundedCornerShape(16.dp), color = MaterialTheme.colorScheme.surface, modifier = Modifier.fillMaxWidth()) {
        Row(modifier = Modifier.padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
            Column(modifier = Modifier.weight(1f)) {
                ...
                Spacer(Modifier.height(8.dp))
                Text(suggestion.title, style = MaterialTheme.typography.titleMedium, color = MaterialTheme.colorScheme.onSurface, fontWeight = FontWeight.SemiBold)
            ...
            IconButton(onClick = { onAccept(suggestion) }) {
                Icon(Icons.Filled.AddCircle, contentDescription = "Accept quest", tint = MaterialTheme.colorScheme.primary, modifier = Modifier.size(36.dp))
            }
        }
    }
}
