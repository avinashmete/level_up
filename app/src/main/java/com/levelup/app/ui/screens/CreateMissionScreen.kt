...
fun CreateMissionScreen(
    ...
    var type by remember {
        mutableStateOf(
            runCatching { MissionType.valueOf(existing?.type ?: MissionType.DAILY.name) }
                .getOrDefault(MissionType.DAILY)
        )
    }
    var stat by remember {
        mutableStateOf(
            runCatching { StatType.valueOf(existing?.stat ?: StatType.DISCIPLINE.name) }
                .getOrDefault(StatType.DISCIPLINE)
        )
    }
    var difficulty by remember {
        mutableStateOf(
            runCatching { Difficulty.valueOf(existing?.difficulty ?: Difficulty.NORMAL.name) }
    ...
    Scaffold(
        ...
        Column(
            ...
            SectionLabel("TYPE")
            ChipRow(
                items = MissionType.values().toList(),
                selected = type,
                labelOf = { it.name },
                colorOf = {
                    when (it) {
            ...
            Button(
                onClick = {
                    viewModel.saveMission(existing, title, description, type, stat, difficulty)
                    onDone()
                },
...
private fun <T> ChipRow(
    ...
    selected: T,
    labelOf: (T) -> String,
    colorOf: @Composable (T) -> androidx.compose.ui.graphics.Color,
    onSelect: (T) -> Unit
) {
...
@Suppress("unused")
@Composable
private fun BoxPad() = Box(Modifier.padding(0.dp))
